...
# ---------------- functions ----------------
def plot_energy(file1d='GYRE_1d_00010101_00301230_series_energy.nc'):
...
def plot_surf_EKE(file360d='GYRE_360d_00010101_00301230_ocean.nc', cmap=cmocean.cm.balance):
    print(file360d)
    ...
    plt.contourf(lon, lat, EKE, 15, vmin=0, cmap=cmap)
    plt.colorbar()
...
def plot_z_EKE(file360d='GYRE_360d_00010101_00301230_ocean.nc', filemask='mesh_mask.nc'):
    ds = nc.Dataset(file360d)
...
def plot_SST(file360d='GYRE_360d_00010101_00301230_ocean.nc'):
    ds = nc.Dataset(file360d)
...
def plot_snapshot_vorticity(file5d = 'GYRE_5d_00010101_00301230_surf.nc'):
    ds = nc.Dataset(file5d)
    w = ds['surf_rot'][-1-72+18,:,:] # 30 March of the last year
    lat = ds['nav_lat'][:]
    lon = ds['nav_lon'][:]
    
    plt.contourf(lon, lat, w, 30, vmin=-0.5, vmax=0.5, cmap=cmocean.cm.balance)
    plt.colorbar()
    plt.xlabel('Longitude')
    plt.ylabel('Latitude') 
...
def plot_flux_meridional(file360d='GYRE_360d_00010101_00301230_ocean.nc', filemask='mesh_mask.nc'):
    llat, depth, Qlat, Q, Psi_MOC = compute_flux(file360d, filemask)
    plt.plot(llat, Qlat)
    plt.xlabel('Latitude')
...
def plot_flux_MOC(file360d='GYRE_360d_00010101_00301230_ocean.nc', filemask='mesh_mask.nc'):
    llat, depth, Qlat, Q, Psi_MOC = compute_flux(file360d, filemask)
...
def compute_flux(file360d='GYRE_360d_00010101_00301230_ocean.nc', filemask='mesh_mask.nc'):
    rau0 = 1026.
